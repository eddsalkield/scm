...
run_test() {
     echo "checking that a general install of a package works as expected, under modified target directories"
     mkdir -p "${TEMP_LOCAL}/new-target"
     exe -d "${BASE_DIR}/test/repo-target" -t "${TEMP_LOCAL}/" -B desktop1 -y install vim
